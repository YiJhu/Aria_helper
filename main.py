                    ...
                    if msg[3] == 2: 
                        if msg[1] in Owner or msg[1] in Admin:
                            if msg[10] == '#help':
                                with concurrent.futures.ThreadPoolExecutor(max_workers=90) as executor:
                                    ...
                                    if msg[1] in Owner:
                                        ...
                                        continue
                                    executor.submit(bot.sendMessage(msg[2], f"【HELP COMMAND】\n{help}"))

                            if msg[10] == '#speed':
                                with concurrent.futures.ThreadPoolExecutor(max_workers=90) as executor:
                                    speed = timeit.timeit('"-".join(str(n) for n in range(100))', number=10000)
                                    executor.submit(bot.sendMessage(msg[2], "SpeedTest： %s s" % (speed), relatedMessageId=msg[4]))

                            if msg[10] == '#time':
                                with concurrent.futures.ThreadPoolExecutor(max_workers=90) as executor:
                                    stime = bot.getServerTime()
                                    executor.submit(bot.sendMessage(msg[2],  "【Now Time (UTC+8)】\n" + time.strftime('%Y-%m-%d %I:%M:%S %p', time.localtime(stime/1000)), relatedMessageId=msg[4]))

                            if msg[10] == '#me':
                                with concurrent.futures.ThreadPoolExecutor(max_workers=90) as executor:
                                    executor.submit(bot.sendContact(msg[2], msg[1], "darkkingtw.cf"))

                            if msg[10] == '#mid':
                                bot.sendMessage(msg[2], msg[1], relatedMessageId=msg[4])

                            if msg[10].startswith("#mid:"): #mid: mentions
                                mlists = []
                                if 'MENTION' in msg[18]:
                                    ...
                                    for mlist in mlists:
                                        ...
                                        txt += f"{n[22]}\n"
                                        txt += f"{n[1]}\n"
                                bot.sendMessage(msg[2], txt, relatedMessageId=msg[4])
                                del mlists

                            if msg[10].startswith("#userinfo:"): #userinfo: mentions
                                mlists = []
                                if 'MENTION' in msg[18]:
                                    key = eval(msg[18]["MENTION"])
                                    tags = key["MENTIONEES"]
                                    ...
                                    for mlist in mlists:
                                        ...
                                        try:
                                            bot.sendMessage(msg[2], 'User Name:\n%s\nUser Mid:\n%s\nStatus Message:\n%s\nProfile Link:\n%s/%s' %(concact[22], concact[1], concact[26], bot.LINE_PROFILE_CDN_DOMAIN, concact[24]), relatedMessageId=msg[4])
                                        except:
                                            bot.sendMessage(msg[2], 'User Name:\n%s\nUser Mid:\n%s\nStatus Message:\n%s\nProfile Link:\n%s/%s' %(concact[22], concact[1], concact[26], bot.LINE_PROFILE_CDN_DOMAIN, 'None'), relatedMessageId=msg[4])

                            if msg[10] == '#gid':
                                bot.sendMessage(msg[2], msg[2])

                            if msg[10].startswith("#getcontact:"): #getcontact:mid
                                key = msg[10][12:]
                                if len(key) <= 32:
                                    bot.sendMessage(msg[2], '【MID ERROR】\nCharacter is empty', relatedMessageId=msg[4])
                                else:
                                    try:
                                        xmid = bot.getContact(key)[2]
                                        try: 
                                            bot.sendContact(msg[2], key, "darkkingtw.cf")
                                        except:
                                            pass
                                    except:
                                        bot.sendMessage(msg[2], '【MID ERROR】\nNo such user', relatedMessageId=msg[4])

                            if msg[10].startswith("#tagcontact:"): #tagcontact: mentions
                                mlists = []
                                if 'MENTION' in msg[18]:
                                    key = eval(msg[18]["MENTION"])
                                    tags = key["MENTIONEES"]
                                    for tag in tags:
                                        ...
                                        if mid[1] not in mlists:
                                            mlists.append(mid[1])
                                    for mlist in mlists:
                                        bot.sendContact(msg[2], mlist, "darkkingtw.cf")
                                del mlists
                            ...
                            if msg[10] == '#ginfo':
                                ...
                                try: g_info += "\n[Group Profile]\n%s/%s" %(bot.LINE_OBS_DOMAIN, gid[11])
                                except: g_info += "\n[Group Profile]\n%s/%s" %(bot.LINE_OBS_DOMAIN, 'None')
                                g_info += "\n[Created Time]\n"+ time.strftime('%Y-%m-%d %I:%M:%S %p', time.localtime(gid[2]/1000))
                                try: g_info += "\n----------------------------------------\nMembers： " + str(len(gid[20])) + "\nPending： " + str(len(gid[22]))
                                except: g_info += "\n----------------------------------------\nMembers： " + str(len(gid[20])) + "\nPending： 0"
                                if gid[12] is False: g_info += "\nInvitation Url： Allowed."
                                else: g_info += "\nInvitation Url： Blocked."
                                bot.sendMessage(msg[2], '%s' %(g_info), relatedMessageId=msg[4])

                            if msg[10] == '#gowner':
                                gid = bot.getGroup(msg[2])
                                try:
                                    bot.sendContact(msg[2], gid[21][1], "darkkingtw.cf")
                                except:
                                    bot.sendMessage(msg[2], "The original group creator has deleted the account.\nThis is the inherited group creator!", relatedMessageId=msg[4])
                                    bot.sendContact(msg[2], gid[20][0][1], "darkkingtw.cf")
                            ...
                            if msg[10].startswith("#url:"): #url:on / off
                                ...
                                if key.lower() == "on":
                                    ...
                                    bot.sendMessage(msg[2], f"https://line.me/R/ti/g/{uri[1]}", relatedMessageId=msg[4])
                                if key.lower() == "off":
                            ...
                            if msg[10].startswith("#regname:"): #regname:str
                                ...
                                bot.sendMessage(msg[2], f'【Re GROUP NAME】\n{key}', relatedMessageId=msg[4])
                                    ...
                                    for tag in tags:
                                        ...
                                        if mid[1] not in klist:
                                            ...
                                            with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
                                                for kmid in klist:
                                                    executor.submit(bot.deleteOtherFromChat(msg[2], kmid))      
                                                    txt = ""
                                                    txt += mid[22]
                                        bot.sendMessage(msg[2], f'【Kick OUT】\n{txt}', relatedMessageId=msg[4])
                                del klist
                                ...
                                try:
                                    ...
                                    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                                        for Cmember in clist:
                                            start = time.time()
                                            executor.submit(bot.cancelChatInvitation(msg[2], Cmember[1]))
                                            time.sleep(0.8)
                                        end = time.time()
                            ...
                            if msg[10].startswith("#data:"): #data:num
                                num = int(msg[10][6:])+1
                                try:
                                    data = bot.getRecentMessagesV2(msg[2])[num-1]
                                    bot.sendMessage(msg[2], f'{data}')
                                except Exception as e:
                                    bot.sendMessage(msg[2], f'{e}', relatedMessageId=msg[4])
                        if msg[1] in Owner:
                            '''for owner'''
                            ...
                            if msg[10].startswith("#rename:"): #rename:str
                                key = msg[10][8:]
                                if len(key) == 0:
                                    bot.sendMessage(msg[2], '【Re NAME ERROR】\nCharacter is empty')
                                else:
                                    bot.updateProfileAttribute(2, key)
                                    bot.sendMessage(msg[2], f'【Re NAME】\n{key}')
                            
                            if msg[10] == '#repic:': #repic: file
                                key = msg[10][7:]
                                ...
                                with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
                                    ...
                                    for kmid in klist:
                                        if not kmid[1] in Owner or not kmid[1] in bot.profile[1]:
                                            executor.submit(bot.deleteOtherFromChat(msg[2], kmid[1]))
                                    end = time.time()
                                bot.sendMessage(msg[2], (end - start))
                ...
                if msg[15] == 6: #call #Not every device is supported. exg:chrome and more...
                    if msg[3] == 2:
                        ...
                        if contentMetadata['GC_MEDIA_TYPE'] in ["AUDIO","VIDEO","LIVE"]:
                            type_voices = stype[0][contentMetadata['GC_CHAT_MID'][:1]]+stype[1][contentMetadata['GC_MEDIA_TYPE']]
                ...
                if msg[15] == 13:
                    if msg[3] == 2:
                        ...
                        with concurrent.futures.ThreadPoolExecutor(max_workers=90) as executor:
                            ...
                            try:
                                executor.submit(bot.sendMessage(msg[2], 'User Name:\n%s\nUser Mid:\n%s\nStatus Message:\n(Only show 100 words!)\n%s\nProfile Link:\n%s/%s' %(someone[22], someone[1], someone[26][:100], bot.LINE_PROFILE_CDN_DOMAIN, someone[24])))
                            except:
                                executor.submit(bot.sendMessage(msg[2], 'User Name:\n%s\nUser Mid:\n%s\nStatus Message:\n(Only show 100 words!)\n%s\nProfile Link:\n%s/%s' %(someone[22], someone[1], someone[26][:100], bot.LINE_PROFILE_CDN_DOMAIN, 'None')))
                    if msg[3] == 0:
                        someone = bot.getContact(msg[18]["mid"])
                        ...
                        keyData = bot.generateSharedSecret(bytes(private_key), receiver_key_data[2][4])
                        specVersion = receiver_key_data[3]
                        try:
                            text = 'User Name:\n%s\nUser Mid:\n%s\nStatus Message:\n(Only show 100 words!)\n%s\nProfile Link:\n%s/%s' %(someone[22], someone[1], someone[26][:100], bot.LINE_PROFILE_CDN_DOMAIN, someone[24])
                            encData = bot.encryptE2EETextMessage(senderKeyId, receiverKeyId, keyData, specVersion, text, msg[1] ,bot.mid)
                            bot.sendMessageWithChunks(msg[1], encData, 0, {'e2eeVersion': '2', 'contentType': '0'})
                        except:
                            text = 'User Name:\n%s\nUser Mid:\n%s\nStatus Message:\n(Only show 100 words!)\n%s\nProfile Link:\n%s/%s' %(someone[22], someone[1], someone[26][:100], bot.LINE_PROFILE_CDN_DOMAIN, 'None')
                            encData = bot.encryptE2EETextMessage(senderKeyId, receiverKeyId, keyData, specVersion, text, msg[1] ,bot.mid)
                            bot.sendMessageWithChunks(msg[1], encData, 0, {'e2eeVersion': '2', 'contentType': '0'})
                ...
                if msg[15] == 14:
                    if msg[3] == 2:
                        ...
                        if 'FILE_NAME' in contentMetadata:
                            ftxt += "\n[File Name]\n%s" %(contentMetadata['FILE_NAME'])
                        if 'FILE_SIZE' in contentMetadata:
                            if int(contentMetadata['FILE_SIZE']) <= 1024:
                                ftxt += "\n[File Size]\n%s B" %(round(int(contentMetadata['FILE_SIZE']), 1))
                            elif int(contentMetadata['FILE_SIZE']) <= 1048576:
                                ftxt += "\n[File Size]\n%s KB" %(round(int(contentMetadata['FILE_SIZE'])/1024, 1))
                            elif int(contentMetadata['FILE_SIZE']) <= 1073741824:
                                ftxt += "\n[File Size]\n%s MB" %(round(int(contentMetadata['FILE_SIZE'])/1048576, 1))
                            else:
                                ftxt += "\n[File Size]\n%s GB" %(round(int(contentMetadata['FILE_SIZE'])/1073741824, 1))
                        if 'FILE_EXPIRE_TIMESTAMP' in contentMetadata:
                            ftxt += "\n[File Expire time]\n%s" %(time.strftime('%Y-%m-%d %I:%M:%S %p', time.localtime(int(contentMetadata['FILE_EXPIRE_TIMESTAMP'])/1000)))
                    ...
                    if msg[3] == 2:
                        ...
                        if contentMetadata['serviceType'] == 'GB': #group post
                            ...
                            if 'mediaOid' in contentMetadata:
                                if contentMetadata['mediaType'] == 'I': #Image
                                    g_post += "\n[Media Url]\n%s/r/myhome/h/%s" %(bot.LINE_OBS_DOMAIN, contentMetadata['mediaOid'])
                        ...
                        if contentMetadata['serviceType'] == 'AB': #album
                            if contentMetadata['locKey'] == 'BA': #creat album
                                album = "【Group Album】"
                                album += "\n[Photo Sender]\n%s" %(bot.getContact(msg[1])[22])
                                if 'albumName' in contentMetadata:
                                    album += "\n[Album Name]\n%s" %(contentMetadata['albumName'])
                                if 'mediaCount' in contentMetadata:
                                    album += "\n[Photo Count]\n%s" %(int(contentMetadata['mediaCount'])+1)
                                album += "\n[Album Url]\n%s" %(contentMetadata['postEndUrl'].replace("line://", "https://line.me/R/"))
                        ...
                        if op[11] in Admin or op[11] in Owner:
                            ...
                            if not os.path.exists(f'data/group/{op[10]}'):
                                ...
                                with open(f'data/group/{op[10]}/setting.json', 'w') as gset:
                                    data = [{"admin": {},"post_event": True,"auto_read" : False,"contact_event": True }]
                                    json.dump(data, gset, sort_keys=True, indent=4)
                                    bot.sendMessage(op[10], 'Initialization succeeded')
                            else:
